...
def create_distance_matrix(locations_):
    ...
    for from_node in range(size):
        ...
        for to_node in range(size):
            x1 = locations_[from_node][0]
            y1 = locations_[from_node][1]
            x2 = locations_[to_node][0]
            y2 = locations_[to_node][1]
            dist_matrix[from_node][to_node] = euclid_distance(x1, y1, x2, y2)
    return dist_matrix


def create_distance_callback(dist_matrix_):
    # Create the distance callback
    def distance_callback(from_ndoe, to_node):
        return int(dist_matrix_[from_ndoe][to_node])
    return distance_callback


def main():
    # create the data
    loc_data = create_data_array()
    dist_matrix_data = create_distance_matrix(loc_data)
    dist_callback = create_distance_callback(dist_matrix_data)

    tsp_size = len(loc_data)
    num_routes = 1
    depot = 0
