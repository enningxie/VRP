...
def main():
    ...
    # declare an array to hold our nutritional
    food = [[]] * len(data)
    ...
    if status == solver.OPTIMAL:
        ...
        nutrients_ = [0] * len(nutrients)

        for i in range(len(data)):
            price += food[i].solution_value()

            for nutrient in range(num_nutrients):
                nutrients_[nutrient] += data[i][nutrient+3] * food[i].solution_value()
            if food[i].solution_value() > 0:
                print('{} = {}'.format(data[i][0], food[i].solution_value()))

        print('Optimal annual price: {:.2f}'.format(price*365))
